...
ifinal = np.where(time == np.datetime64('2010-08-08T07:00:00.000000000'))[0][0]

inputfile = 'scratch/rvorticity/rvor_identify/re_identify/identified_transformed_rvor_20100803_09_stricter_dir.h5'
outputfile = 'figures/09_decades_vortex/09_03_require_theta_anomaly/9_3_2.2 Re_Identified transformed rvor 20100803_09_003_formal_1day.mp4'
...
for i in np.arange(istart, ifinal):  # np.arange(istart, istart + 2):  #
    # i = istart + 5
    ################################ import vorticity
    ...
    for j in range(vortex_count):
        ...
        ################################ plot text
        #### plot P1
        if (str(time[i])[0:13] in track_p1.keys()):
            if (j in track_p1[str(time[i])[0:13]]):
                plt_textp1 = ax.text(
                    center_lon, center_lat, 'P1',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textp1)
        
        #### plot P2
        if (str(time[i])[0:13] in track_p2.keys()):
            if (j in track_p2[str(time[i])[0:13]]):
                plt_textp2 = ax.text(
                    center_lon, center_lat, 'P2',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textp2)
        
        #### plot P3
        if (str(time[i])[0:13] in track_p3.keys()):
            if (j in track_p3[str(time[i])[0:13]]):
                plt_textp3 = ax.text(
                    center_lon, center_lat, 'P3',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textp3)
        
        #### plot P4
        if (str(time[i])[0:13] in track_p4.keys()):
            if (j in track_p4[str(time[i])[0:13]]):
                plt_textp4 = ax.text(
                    center_lon, center_lat, 'P4',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textp4)
        
        #### plot N1
        if (str(time[i])[0:13] in track_n1.keys()):
            if (j in track_n1[str(time[i])[0:13]]):
                plt_textn1 = ax.text(
                    center_lon, center_lat, 'N1',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textn1)
        
        #### plot N2
        if (str(time[i])[0:13] in track_n2.keys()):
            if (j in track_n2[str(time[i])[0:13]]):
                plt_textn2 = ax.text(
                    center_lon, center_lat, 'N2',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textn2)
        
        #### plot N3
        if (str(time[i])[0:13] in track_n3.keys()):
            if (j in track_n3[str(time[i])[0:13]]):
                plt_textn3 = ax.text(
                    center_lon, center_lat, 'N3',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textn3)
        
        #### plot N4
        if (str(time[i])[0:13] in track_n4.keys()):
            if (j in track_n4[str(time[i])[0:13]]):
                plt_textn4 = ax.text(
                    center_lon, center_lat, 'N4',
                    color='m', size=8, fontweight='bold')
                vortices_plot.append(plt_textn4)
    ...
    ims.append(add_arts + [plt_rvor, rvor_time] + vortices_plot)
...
cbar.ax.set_xlabel("100-meter relative vorticity [$10^{-4}\;s^{-1}$]")
...
ani.save(
    'figures/09_decades_vortex/09_03_require_theta_anomaly/9_3_2.3 Re_Identified transformed rvor 20100803_09_003_formal_1day_250.mp4',
    progress_callback=lambda i, n: print(f'Saving frame {i} of {n}'),)
